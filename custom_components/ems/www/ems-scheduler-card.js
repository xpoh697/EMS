...
class EmsSchedulerCard extends HTMLElement {
        ...
        <div id="stats-view" class="hidden">
          ...
          <div class="chart-container" style="margin-bottom: 24px;">
            ...
            <div class="chart-legend" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; justify-items: start; margin-top: 12px; padding: 0 8px;">
              ...
              <div class="legend-item">
                <div class="legend-color" style="background: #ff5252; height: 2px; border-radius: 0;"></div>
                <span>Buy Price</span>
              </div>
              <div class="legend-item">
                <div class="legend-color" style="background: #4caf50; height: 2px; border-radius: 0;"></div>
  ...
  _drawHistoryChart() {
    ...
    const barW = 14.0; // single wide column width
    ...
    for (let i = 0; i < dayData.length; i++) {
      ...
      if (pvH > 1.2) {
        barsHtml += `<rect x="${xSlot.toFixed(1)}" y="${(chartCenterY - pvH).toFixed(1)}" width="${barW}" height="${pvH.toFixed(1)}" fill="url(#pv-grad-${this._cardId})" rx="2" ry="2" opacity="0.75"/>`;
      }

      // Import (blue, going up - overlays on top of PV Gen at same coordinate)
      if (impH > 1.2) {
        barsHtml += `<rect x="${xSlot.toFixed(1)}" y="${(chartCenterY - impH).toFixed(1)}" width="${barW}" height="${impH.toFixed(1)}" fill="url(#import-grad-${this._cardId})" rx="2" ry="2" opacity="0.75"/>`;
      }

      // Export (red, going down from zero-line)
      if (expH > 1.2) {
        barsHtml += `<rect x="${xSlot.toFixed(1)}" y="${chartCenterY.toFixed(1)}" width="${barW}" height="${expH.toFixed(1)}" fill="url(#export-grad-${this._cardId})" rx="2" ry="2" opacity="0.75"/>`;
      }
    }
    ...
    container.innerHTML = `
      <svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block;overflow:visible;">
        ...
        <path d="M ${socPoints.join(' L ')}" fill="none" stroke="#4dd0e1" stroke-width="1.5"/>
        <path d="M ${buyPoints.join(' L ')}" fill="none" stroke="#ff5252" stroke-width="1.5" stroke-dasharray="3 3"/>
        <path d="M ${sellPoints.join(' L ')}" fill="none" stroke="#4caf50" stroke-width="1.5" stroke-dasharray="3 3"/>
